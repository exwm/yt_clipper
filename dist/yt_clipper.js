...
(function () {
    ...
    let fps;
    try {
        let playerApiScript = document.querySelectorAll('#player > script:nth-child(3)')[0].textContent;
    ...
    markers.toString = function () {
        ...
        this.forEach((markerPair, idx) => {
            markersString += `${markerPair[0]},${markerPair[1]},${markerPair[2]},'${markerPair[3]}',`;
...
#speed-input:invalid, #crop-input:invalid, #res-input:invalid {
  ...
  `;
    ...
    function toggleDefaultSettings() {
      ...
      <input id="speed-input" type="number" placeholder="speed" value="${settings.defaultSlowdown}" step="0.1" min="0.1" max="8" style="width:4em">
      <span style="color:grey;font-size:12pt;font-style:italic"> Default Slowdown - </span>
      <input id="crop-input" value="${settings.defaultCrop}" pattern="${cropInputValidation}" style="width:10em">
      <span style="color:grey;font-size:12pt"> Default Crop - </span>
      <input id="res-input" list="resolutions"pattern="${resInputValidation}" value="${settings.videoRes}" style="width:7em">
    ...
    function addInputListeners(inputs) {
        inputs.forEach(input => {
            ...
            inputElem.addEventListener('change', e => updateDefaultValue(e, updateTarget), false);
        });
    }
    function updateDefaultValue(e, updateTarget) {
        ...
        if (settings[updateTarget] === settings.defaultCrop ||
    ...
    function resizeCropOverlay(cropDiv) {
        ...
        cropDiv.setAttribute('style', `width:${videoRect.width}px;height:${videoRect.height}px;left:${videoRect.left}px;top:${videoRect.top}px;position:absolute`);
    }
    ...
    function toggleOverlay() {
        ...
        if (cropSvg) {
            ...
            if (cropDivDisplay === 'none')
                cropSvg.setAttribute('display', 'block');
            else {
    ...
    function beginDraw(e, videoRect) {
        if (e.button == 0 && e.shiftKey && !e.ctrlKey && !e.altKey) {
            const beginX = Math.round(((e.pageX - videoRect.left) / videoRect.width) * settings.videoWidth);
            const beginY = Math.round(((e.pageY - 56) / videoRect.height) * settings.videoHeight);
            let crop = `${beginX}:${beginY}:`;
            playerInfo.video.addEventListener('mousedown', e => endDraw(e, crop, beginX, beginY, videoRect), { once: true, capture: true });
        }
        else {
            togglePlayerControls();
        }
    ...
    function endDraw(e, crop, beginX, beginY, videoRect) {
        if (e.button == 0 && e.shiftKey && !e.ctrlKey && !e.altKey) {
            const endX = Math.round(((e.pageX - videoRect.left) / videoRect.width) * settings.videoWidth);
    ...
    function toggleMarkerEditor(e) {
        ...
        <span id='end-time'>${currentMarkerTime}</span></div>`;
            infoContents.insertBefore(markerInputs, infoContents.firstChild);
            addMarkerInputListeners([['speed-input', 'slowdown'], ['crop-input', 'crop']], currentMarker, currentMarkerTime, currentIdx);
            isMarkerEditorOpen = true;
            wasDefaultsEditorOpen = false;
        }
    }
    function enableMarkerHotkeys(endMarker) {
        ...
        enableMarkerHotkeys.deleteMarkerPair = () => {
            ...
            enableMarkerHotkeys.endMarker.parentElement.removeChild(enableMarkerHotkeys.endMarker);
            enableMarkerHotkeys.startMarker.parentElement.removeChild(enableMarkerHotkeys.startMarker);
    ...
    function addMarkerInputListeners(inputs, currentMarker, currentMarkerTime, currentIdx) {
        inputs.forEach(input => {
            ...
            inputElem.addEventListener('focus', e => (toggleKeys = false), false);
            inputElem.addEventListener('blur', e => (toggleKeys = true), false);
            inputElem.addEventListener('change', e => updateMarker(e, updateTarget, currentMarker, currentMarkerTime, currentIdx), false);
        });
    }
...
title = re.sub("'","", r'''${document.getElementsByClassName('title')[0].lastElementChild.textContent}''')
    ...
    function buildGfyRequestPromise(req, idx, accessToken) {
        ...
        return new Promise((resolve, reject) => {
            postData('https://api.gfycat.com/v1/gfycats', req, accessToken)
                .then(resp => {
                links.push(`(${settings.shortTitle}-${idx})[https://gfycat.com/${resp.gfyname}${req.speed}]`);
